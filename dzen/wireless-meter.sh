...
# format the values to 4 digit fields
  ...
  if [ x"$WIRELESS" = x"yes" ]; then
    SIGNAL=$( /usr/sbin/iwconfig $INTERFACE | sed -ne '/Link Quality/ { s/.*Link Quality:\([0-9]*\)\/\([0-9]*\).*/\1/p}' )
    #SIGNALMETER=`echo $SIGNAL | gdbar -s v -h 15 -ss 1 -sh 2 -sw 12 -fg $BFG \
    SIGNALMETER=`echo $SIGNAL | gdbar -h 15 -fg $BFG \
                -bg $BBG -nonl`
    WLAN_DETAILS=$(/usr/sbin/iwconfig $INTERFACE | awk '
...
# reset old rates
  ...
  sleep $SLEEP
       ...
       -y $Y -w $WIDTH -e 'entertitle=uncollapse;leavetitle=collapse' -e 'onstart=lower,collapse;onstart=hide;sigusr1=togglehide;sigusr2=unhide' -fn 'fixed'
